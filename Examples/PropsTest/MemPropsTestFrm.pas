...
begin
  ...
  zDrawEngine.DrawCommand.FillRect(zDrawEngine.ScreenRect, vec4(0.5, 0.5, 0.5, 1));
  ...
  for i := 0 to DrawList.Count - 1 do
    begin
      ...
      v := TDE4V.Init(d.bio.Position, d.bio.Radius * 2, d.bio.Radius * 2, FinalAngle(d.bio.RollAngle) - 90);
      d.alpha := umlProcessCycleValue(d.alpha, 1 * zDrawEngine.LastDeltaTime, 1, 0.1, d.alpha_f);

      zDrawEngine.DrawCommand.SetLineWidth(1);
      zDrawEngine.DrawCommand.DrawRect(v.BoundRect, vec4(0.1, 0.3, 0.1, d.alpha));
      ...
      zDrawEngine.DrawCommand.DrawLine(d.bio.Position, PointRotation(d.bio.Position, d.bio.Radius * 1.5, FinalAngle(d.bio.RollAngle)), vec4(1, 0.3, 0.3, d.alpha));
