...
uses System.Math.Vectors, System.Math,
  ...
  PascalStrings,
  CoreClasses, zDrawEngine, UnicodeMixedLib, Geometry2DUnit,
  MemoryRaster, DataFrameEngine;
...
begin
  with TPolyRect.Init(sour.MakeRectV2, sour.Angle) do
    begin
...
procedure TDrawEngineInterface_FMX.DrawRect(r: TDERect; color: TDEColor);
begin
  FCanvas.Stroke.color := c2c(color);
  FCanvas.fill.color := FCanvas.Stroke.color;
...
procedure TDrawEngineInterface_FMX.FillRect(r: TDERect; color: TDEColor);
begin
  FCanvas.Stroke.color := c2c(color);
  FCanvas.fill.color := FCanvas.Stroke.color;
...
procedure TDrawEngineInterface_FMX.DrawText(text: SystemString; size: TDEFloat; r: TDERect; color: TDEColor; center: Boolean);
var
  ta: TTextAlign;
begin
  FCanvas.Stroke.color := c2c(color);
...
begin
  ...
  if (t is TDETexture_FMX) then
      ...
      if not IsEqual(newDest.Angle, 0, 0.1) then
        begin
          ...
          MakeMatrixRotation(newDest.Angle, newDest.Width, newDest.Height,
            MinValue(newDest.Left, newDest.Right), MinValue(newDest.Top, newDest.Bottom), m, r);
          FCanvas.SetMatrix(m);
    ...
    begin
      ...
      if not IsEqual(newDest.Angle, 0, 0.1) then
        begin
          bak := FCanvas.Matrix;
          MakeMatrixRotation(newDest.Angle, newDest.Width, newDest.Height, MinValue(newDest.Left, newDest.Right), MinValue(newDest.Top, newDest.Bottom), m, r);
          FCanvas.SetMatrix(m);
  ...
  if FDebug then
    begin
      ...
      FCanvas.DrawRect(Geometry2DUnit.TPolyRect.Init(newDest.MakeRectV2, newDest.Angle).BoundRectf, 0, 0, [], 0.3);
    end;
end;
