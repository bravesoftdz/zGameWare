...
begin
  ...
  {$IFDEF parallel}
  {$IFDEF FPC}
  ProcThreadPool.DoParallelLocalProc(@Nested_ParallelFor, 0, (Width * Height) - 1);
  {$ELSE FPC}
  TParallel.For(0, (Width * Height) - 1, procedure(pass: Integer)
    begin
      dest.FBits^[pass] := RGBA2BGRA(FBits^[pass]);
    end);
  {$ENDIF FPC}
...
procedure TMemoryRaster.FormatBGRA;
  ...
  procedure Nested_ParallelFor(pass: PtrInt; Data: Pointer; Item: TMultiThreadProcItem);
  begin
    FBits^[pass] := RGBA2BGRA(FBits^[pass]);
  end;
{$ENDIF FPC}
{$IFNDEF parallel}
  procedure DoFor;
