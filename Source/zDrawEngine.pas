...
type
  ...
  IDrawEngineInterface = interface
    ...
    procedure DrawRect(r: TDERect; color: TDEColor);
    procedure FillRect(r: TDERect; color: TDEColor);
    procedure DrawEllipse(r: TDERect; color: TDEColor);
    procedure FillEllipse(r: TDERect; color: TDEColor);
    procedure DrawText(text: SystemString; size: TDEFloat; r: TDERect; color: TDEColor; Center: Boolean);
  ...
  public
    ...
    procedure DrawRect(r: TDERect; color: TDEColor); {$IFDEF INLINE_ASM}inline; {$ENDIF}
    procedure FillRect(r: TDERect; color: TDEColor); {$IFDEF INLINE_ASM}inline; {$ENDIF}
    ...
    procedure DrawText(text: SystemString; size: TDEFloat; r: TDERect; color: TDEColor; Center: Boolean); {$IFDEF INLINE_ASM}inline; {$ENDIF}
  ...
  public
    constructor Create(ADrawInterface: IDrawEngineInterface); virtual;
    destructor Destroy; override;
    ...
    { }
    procedure DrawBox(r: TDERect; color: TDEColor; LineWidth: TDEFloat); {$IFDEF INLINE_ASM}inline; {$ENDIF}
    procedure DrawBoxInScene(r: TDERect; color: TDEColor; LineWidth: TDEFloat); {$IFDEF INLINE_ASM}inline; {$ENDIF}
    { }
    procedure FillBox(r: TDERect; color: TDEColor); {$IFDEF INLINE_ASM}inline; {$ENDIF}
    procedure FillBoxInScene(r: TDERect; color: TDEColor); {$IFDEF INLINE_ASM}inline; {$ENDIF}
    { }
    procedure DrawEllipse(pt: TDEVec; radius: TDEFloat; color: TDEColor); overload; {$IFDEF INLINE_ASM}inline; {$ENDIF}
    procedure DrawEllipse(r: TDERect; color: TDEColor); overload; {$IFDEF INLINE_ASM}inline; {$ENDIF}
...
begin
  Result := Geometry2DUnit.TPolyRect.Init(MakeRectV2, Angle).BoundRect;
end;

function TDE4V.Centroid: TDEVec;
begin
  Result := Geometry2DUnit.TPolyRect.Init(MakeRectV2, Angle).Centroid;
end;
...
begin
  r := MakeRectV2;
  r[0] := PointAdd(r[0], X, Y);
  r[1] := PointAdd(r[1], X, Y);
  Result := Init(r, Angle);
end;
  ...
  try
    case t of
      ...
      dctDrawText: with PDrawCommandParam_Textout(data)^ do
            IDraw.DrawText(text, size, r, color, Center);
...
procedure TDrawQueue.DrawRect(r: TDERect; color: TDEColor);
var
  p: PDrawCommand;
...
procedure TDrawQueue.FillRect(r: TDERect; color: TDEColor);
var
  p: PDrawCommand;
  data: PDrawCommandParam_Rect_Color;
begin
...
procedure TDrawQueue.DrawText(text: SystemString; size: TDEFloat; r: TDERect; color: TDEColor; Center: Boolean);
var
  PrepareDraw: Boolean;
  p: PDrawCommand;
...
begin
  ...
  if Downed then
    begin
      r := Button;
      r[0] := PointAdd(r[0], PointMake(2, 2));
      r[1] := PointAdd(r[1], PointMake(2, 2));
    end
  else
      r := Button;
  ...
  c := DEColor(0, 0, 0, 0.0);
  Owner.FDrawCommand.FillRect(r, c);
  c := DEColor(1, 1, 1, 1);
  Owner.FDrawCommand.DrawRect(r, c);
  Owner.FDrawCommand.DrawText(text, textsize, r, c, True);
end;
    ...
    begin
      ...
      if p^.CurrentTime > LifeTime then
        ...
        begin
          ...
          p^.Position := MovementDistance(p^.Position, PointAdd(p^.Position, Dispersion), (k + k * p^.Acceleration) * deltaTime);
...
var
  pr: TPolyRect;
begin
  pr := TPolyRect.Init(d.MakeRectV2, d.Angle);
...
begin
  FDrawCommand.SetLineWidth(LineWidth);
  FDrawCommand.DrawRect(r, color);
end;

procedure TDrawEngine.FillBox(r: TDERect; color: TDEColor);
begin
  FDrawCommand.FillRect(r, color);
end;
...
var
  ...
  box: TDERect;
begin
  FDrawCommand.DrawText(text, size, r, color, Center);
    ...
    begin
      ...
      if Center then
        begin
          ...
          box[0] := PointAdd(box[0], r[0]);
          box[1] := PointAdd(box[0], siz);
          DrawBox(box, color, 1);
        end
      else
        begin
...
begin
  if indentEndge then
      Result := RectEndge(DestScreen, PointMul(RectSize(DestScreen), -0.05))
  else
      Result := DestScreen;
...
begin
  if indentEndge then
      Result := RectEndge(destScene, PointMul(RectSize(destScene), -0.05))
  else
      Result := destScene;
...
begin
  if indentEndge then
      d := RectEndge(DestScreen, PointMul(RectSize(DestScreen), -0.05))
  else
      d := DestScreen;
...
begin
  if indentEndge then
      d := RectEndge(destScene, PointMul(RectSize(destScene), -0.05))
  else
      d := destScene;
...
begin
  ...
  while i >= 0 do
    begin
      ...
      if st.LifeTime > 0 then
        begin
          r[0] := PointSub(pt, st.textRectSize);
          r[1] := PointAdd(r[0], st.textRectSize);
  ...
  if devpFrameEndge in FViewOptions then
    begin
      ...
      FDrawCommand.DrawRect(MakeRect(1, 1, Width - 1, Height - 1), FScreenFrameColor);
    end;
...
begin
  ...
  new(p);
  p^.DrawEng := FDefaultDrawEngineClass.Create(nil);
  p^.DrawEng.ViewOptions := [];
  p^.workObj := workObj;
  p^.LastActivted := GetTimeTick;
  FDrawEngineList.Add(p);
